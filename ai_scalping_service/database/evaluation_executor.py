"""
Evaluation Trading Executor for AI Scalping Service
====================================================
...
class ScalpingEvaluationDatabase:
    """
    Dedicated SQLite database for AI Scalping evaluation mode
    ...
    def __init__(self, db_path: str = None):
        if db_path is None:
            ...
            db_path = str(db_dir / "evaluation_data.db")
        
        self.db_path = db_path
        self._init_database()
        logger.info(f"Scalping evaluation database initialized: {db_path}")
    ...
    def _init_database(self):
        ...
        # Market snapshots during evaluation
        cursor.execute("""
        ...
        # Create indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_scalp_orders_trade ON evaluation_orders(trade_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_scalp_trades_date ON evaluation_trades(timestamp_entry)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_scalp_trades_status ON evaluation_trades(status)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_scalp_signals_date ON signal_decisions(timestamp)")
    ...
    def set_execution_mode(self, mode: ExecutionMode, updated_by: str = "api", notes: str = "") -> bool:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Scalping execution mode changed to: {mode.value} by {updated_by}")
        return True
    ...
    def save_order(self, order: EvaluationOrder) -> int:
        ...
        cursor.execute("""
            INSERT INTO evaluation_orders (
                order_id, trade_id, timestamp, instrument,
                transaction_type, order_type, quantity, requested_price,
                simulated_fill_price, slippage_applied, status, correlation_id, order_payload
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            ...
            order.correlation_id, json.dumps(order.order_payload)
        ))
        conn.commit()
    ...
    def save_trade(self, trade: EvaluationTrade) -> int:
        ...
        cursor.execute("""
            INSERT INTO evaluation_trades (
                trade_id, timestamp_entry, instrument, direction,
                entry_price, quantity, status, entry_reason,
                signal_strength, ai_confidence, momentum_score
            ) VALUES (?, ?, ?, ?, ?, ?, 'open', ?, ?, ?, ?)
        """, (
            trade.trade_id, trade.timestamp_entry, trade.instrument, trade.direction,
            trade.entry_price, trade.quantity, trade.entry_reason,
            trade.signal_strength, trade.ai_confidence, trade.momentum_score
        ))
        conn.commit()
        last_id = cursor.lastrowid
        conn.close()
    ...
    def save_signal_decision(self, decision: Dict) -> int:
        ...
        cursor.execute("""
            INSERT INTO signal_decisions (
                ...
                momentum_score, ai_confidence, was_executed, skip_reason, market_price
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            datetime.now().isoformat(),
    ...
    def save_market_snapshot(self, snapshot: Dict) -> int:
        """Save a market snapshot"""
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO market_snapshots (
                timestamp, instrument, ltp, momentum_score, volume_ratio, phase
            ) VALUES (?, ?, ?, ?, ?, ?)
        """, (
            datetime.now().isoformat(),
            snapshot.get('instrument', ''),
            snapshot.get('ltp', 0),
            snapshot.get('momentum_score', 50),
            snapshot.get('volume_ratio', 1.0),
            snapshot.get('phase', 'DORMANT')
        ))
        conn.commit()
        last_id = cursor.lastrowid
        conn.close()
    ...
    def get_trades_by_date(self, target_date: date) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT * FROM evaluation_trades WHERE DATE(timestamp_entry) = ?
    ...
    def get_daily_summary(self, target_date: date = None) -> Dict:
        ...
        if row:
            ...
            return {
                ...
                'avg_momentum_score': row['avg_momentum_score'] or 0
            }
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        if row:
            ...
            return {
                ...
                'win_rate': (wins / (total - (row['open_trades'] or 0)) * 100) if (total - (row['open_trades'] or 0)) > 0 else 0,
                'total_pnl': row['total_pnl'] or 0,
...
class ScalpingEvaluationExecutor:
    """
    Evaluation executor for scalping trades
    Simulates trade execution without sending to exchange
    """
    
    def __init__(self, initial_capital: float = 100000):
        self._db = ScalpingEvaluationDatabase()
        ...
        logger.info(f"Scalping Evaluation Executor initialized. Session: {self._session_id}")
    ...
    def _load_open_positions(self):
        ...
        for trade in open_trades:
            self._positions[trade['trade_id']] = EvaluationTrade(
                ...
                momentum_score=trade.get('momentum_score', 0)
            )
    ...
    def enter_position(
        ...
        trade_id = f"EVAL-{self._session_id}-{len(self._positions)+1:04d}"
        ...
        logger.info(f"[EVAL] Entered {direction} {instrument} @ {entry_price} | ID: {trade_id}")
    ...
    def exit_position(
        ...
        logger.info(f"[EVAL] Exited {trade.instrument} @ {exit_price} | PnL: {pnl:.2f} ({pnl_percent:.2f}%)")
        
        return pnl
    
    def update_position_mfe_mae(self, trade_id: str, current_price: float):
        """Update MFE/MAE for a position"""
        if trade_id in self._positions:
            trade = self._positions[trade_id]
            self._db.update_trade_mfe_mae(trade_id, current_price, trade.entry_price, trade.direction)
...
# Singleton instance
_scalping_evaluation_executor: Optional[ScalpingEvaluationExecutor] = None


def get_scalping_evaluation_executor() -> Optional[ScalpingEvaluationExecutor]:
    """Get or create the scalping evaluation executor"""
    global _scalping_evaluation_executor
    if _scalping_evaluation_executor is None:
        _scalping_evaluation_executor = ScalpingEvaluationExecutor()
    return _scalping_evaluation_executor


def create_scalping_evaluation_executor(initial_capital: float = 100000) -> ScalpingEvaluationExecutor:
    """Create a new scalping evaluation executor"""
    global _scalping_evaluation_executor
    _scalping_evaluation_executor = ScalpingEvaluationExecutor(initial_capital)
    return _scalping_evaluation_executor
